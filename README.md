...
| [4](https://velog.io/@yapp/03WAF-%EB%A3%B0-5%EB%8B%A8%EA%B3%84%EB%A5%BC-Terraform%EC%9C%BC%EB%A1%9C-%EC%BD%94%EB%93%9C%ED%99%94%ED%95%98%EB%8B%A4) | AWS WAF v2 인프라 코드화(IaC) 및 tfsec 정적 분석 기반의 컴플라이언스 자동화 | WAF Priority 설계, tfsec 정적 분석, Terraform 모듈화 | Security |
| [5](https://velog.io/@yapp/%EB%B3%B4%EC%95%88%EC%9D%80-%EB%B0%B0%ED%8F%AC-%EC%A0%84%EC%97%90-%EC%8B%9C%EC%9E%91%EB%90%9C%EB%8B%A4-AWS-DevSecOps-CICD-%EC%84%A4%EA%B3%84%EA%B8%B0) | GitHub Actions OIDC 기반의 무자격증명(Credentialless) CI/CD 인프라 구축 | OIDC, Trivy·Bandit 보안 게이트, tfstate 원격 백엔드 | Security |
...
AWS WAF 정적 룰의 사각지대를 보완하기 위해 **Isolation Forest** 기반 이상 탐지를 SOAR 파이프라인에 연결했습니다.  
GeoIP·룰 코드·URI/쿼리스트링 Shannon Entropy 등 5개 피처로 학습(공격 400건 + 정상 1,400건 = 1,800건)했고, **Recall 100% (FN=0) / Precision 30.6%** — 공격 미탐 방지를 우선한 2차 보조 탐지 레이어로 설계했습니다.  
피처 설계·평가 근거는 [블로그 시리즈](https://velog.io/@yapp)에 기록했습니다.
