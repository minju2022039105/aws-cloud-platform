...
| [3편](https://velog.io/@yapp/03WAF-%EB%A3%B0-5%EB%8B%A8%EA%B3%84%EB%A5%BC-Terraform%EC%9C%BC%EB%A1%9C-%EC%BD%94%EB%93%9C%ED%99%94%ED%95%98%EB%8B%A4) | WAF 룰 5단계를 Terraform으로 코드화하다 — tfsec이 잡아낸 것들 | tfsec 수동 보안 점검, WAF Priority 설계 |
| [4편](https://velog.io/@yapp/WAF%EA%B0%80-%EB%86%93%EC%B9%9C-%EA%B2%83%EC%9D%84-AI%EA%B0%80-%EC%9E%A1%EB%8A%94-%EB%B2%95-Isolation-Forest-Shannon-Entropy-%EA%B7%B8%EB%A6%AC%EA%B3%A0-%EC%A0%95%EC%A7%81%ED%95%9C-%ED%95%9C%EA%B3%84) | WAF가 놓친 것을 AI가 잡는 법 — Isolation Forest, Shannon Entropy, 그리고 한계 | Isolation Forest, 피처 엔지니어링, 모델 평가 |
...
| [7편](https://velog.io/@yapp/07.-1.-%EC%84%9C%EB%B2%84%EB%A6%AC%EC%8A%A4-%ED%8F%AC%ED%8A%B8%ED%8F%B4%EB%A6%AC%EC%98%A4%EC%97%90-Kubernetes-%EB%8D%94%ED%95%98%EA%B8%B0-kind%EB%B6%80%ED%84%B0-EKS%EA%B9%8C%EC%A7%80) | Kubernetes AI 추론 서버 컨테이너 배포 | FastAPI 컨테이너화, kind/EKS 배포, Helm, Trivy 이미지 스캔 |

---

## Architecture

![전체 아키텍처](docs/architecture/최종아키텍처.png)
...
![CI/CD 파이프라인](docs/architecture/스크린샷/CD%20파이프라인.png)
...
| **Bandit** | Python Lambda 코드 분석 | Python 보안 취약점 정적 분석 |

main 브랜치 push 시 이 두 검사를 통과해야 Terraform apply와 Lambda 배포가 진행됩니다.

---

### Manual IaC Security Audit: tfsec

tfsec은 CI/CD 자동 게이트가 아닌 로컬 수동 감사 도구로 사용했습니다.

초기에는 CI Security Gates에 포함을 시도했으나 워크플로우 안정성 문제가 반복되어 최종 파이프라인에서는 제외했습니다. 대신 배포 전 Terraform 코드 수동 점검과 취약점 개선 이력 관리 목적으로 활용했습니다.
...
| 도구 | 특징 | 판단 |
| :--- | :--- | :--- |
| **tfsec** ✅ | Terraform 전용, AWS 특화 룰 풍부 | Terraform + AWS 단일 스택에 최적 |
| Checkov | Terraform/K8s/CloudFormation 다중 지원 | 범용성이 높지만, 이번 프로젝트는 Terraform + AWS 단일 스택이라 tfsec이 더 단순하게 적용 가능 |
...
![CloudWatch AI 탐지 메트릭](docs/architecture/스크린샷/클라우드%20와치_metric%20그래프%20.png)
...
| 탐지 결과 | 이상 후보 327건 / 21.8% (eval 1,500건 기준) |
...
| 건당 처리 속도 | **0.016ms** | 모델 추론 연산 기준 — 실시간 처리 가능한 수준 |

**Before / After 비교**

| 구분 | 건수 | 의미 |
| :--- | :---: | :--- |
| WAF 정적 룰 차단 | 100건 | SQLi 룰 기반 차단 |
| AI 이상 후보 선별 | 227건 | WAF가 허용한 트래픽 중 추가 검토 대상으로 분류 |
| 총 보안 검토 대상 | **327건** | WAF 차단 + AI 이상 후보 합산 |

> Recall 100% / FN=0: 공격 미탐 방지를 우선하는 보수적 탐지 전략입니다. FP 227건은 실제 공격 여부를 운영자가 검토해야 합니다. WAF 정적 룰의 사각지대를 보완하는 2차 레이어로 설계됐습니다.

![AI 탐지 성능 평가](docs/architecture/스크린샷/AI%20Detection.png)

![AI 보안 대시보드](docs/architecture/스크린샷/스크린샷%20ai대시보드%20.png)
...
![WAF 보안 관제 대시보드](docs/architecture/스크린샷/WAF%20대시보드.png)
...
```hcl
# main 브랜치에서 발급된 OIDC 토큰만 역할 Assume 가능하도록 제한
Condition = {
  StringEquals = {
    "token.actions.githubusercontent.com:sub" =
      "repo:minju2022039105/aws-devsecops-platform:ref:refs/heads/main"
...
| **AI/ML** | Scikit-learn (Isolation Forest), Weighted Federated Averaging (설계·실험) |
| **Compliance** | AWS Config (Rules 11개), GuardDuty, IAM Password Policy |
| **Infrastructure / CI/CD** | Terraform, GitHub Actions (OIDC), Trivy, Bandit, tfsec (수동 점검), KMS, CloudTrail |
...
![Trivy 이미지 스캔](docs/architecture/스크린샷/Security%20Gates.png)

![Kubernetes /predict API 호출](docs/architecture/스크린샷/kub_predict%20API.png)
...
| :--- | :--- | :--- |
| WAF 로그 → AI 추론 경로 | `monitor.py`는 CSV 기반 시뮬레이션 입력 생성기. S3 → Lambda → WAF IP Set 자동화 파이프라인은 end-to-end로 검증됨 | Kinesis Firehose 또는 Lambda를 통한 실제 WAF 로그 직접 연동 |
| 탐지 정밀도 | Precision 30.6% — FP 후보는 운영자 검토 전제 (보수적 탐지 전략) | 정상 트래픽 라벨 확대 후 `contamination` 재튜닝, 피처 보강 |
| Federated Learning | 가중 평균 집계 알고리즘 설계·실험 단계 (단일 환경 검증) | 실제 분산 노드 환경에서의 운영 검증 |
| 데이터 반영 지연 | Athena 기반 준실시간 (Firehose 5~15분 버퍼) | 긴급 대응은 CloudWatch 메트릭(1분) 경로로 보완 중 |
| 평가 데이터 범위 | SQLi 100건 + 정상 1,350건 — 제한된 시나리오 기반 평가 | 다양한 공격 유형(XSS, RCE 등) 추가 후 일반화 성능 재측정 |
