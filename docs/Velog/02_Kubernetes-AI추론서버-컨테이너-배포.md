...
이 프로젝트는 AWS Lambda + API Gateway 기반 서버리스 구조로 운영됩니다.
서버리스는 이벤트 기반 워크로드에 적합하지만, 상시 실행이 필요한 워크로드나 복잡한 컨테이너 오케스트레이션이 요구될 때는 Kubernetes 기반 배포 전략이 필요합니다.

기존 서버리스 레이어와 분리해 `kubernetes-extension/` 디렉토리를 구성했습니다.
컨테이너 이미지 빌드, Rolling Update/Rollback, Helm 패키징, EKS 실배포까지 컨테이너 운영 자동화 흐름을 구현했습니다.
...
`values.yaml`에서 환경별 값을 분리해 dev/prod 배포를 다르게 구성할 수 있다는 점이 핵심입니다.
