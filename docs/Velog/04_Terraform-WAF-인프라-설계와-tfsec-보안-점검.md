# AWS WAF v2 인프라 코드화(IaC) 및 tfsec 정적 분석 기반의 컴플라이언스 자동화

> 이 글은 **AWS Cloud Infrastructure Platform 구축기** 시리즈의 4편입니다.
> WAF 룰 5단계를 Terraform으로 코드화하고, tfsec 정적 분석으로 배포 전 컴플라이언스를 검증한 IaC 설계 과정을 다룹니다.
>
> 이전 글: [3편 — [FinOps] 614만 건의 KMS API 호출 분석을 통한 아키텍처 재설계 및 인프라 비용 92% 절감](https://velog.io/@yapp/5.-614%EB%A7%8C-%EA%B1%B4%EC%9D%98-KMS-%ED%98%B8%EC%B6%9C-%EB%AC%B4%ED%95%9C%EB%A3%A8%ED%94%84%EA%B0%80-30-%EC%B2%AD%EA%B5%AC%EC%84%9C%EB%A5%BC-%EB%A7%8C%EB%93%A0-%EB%82%A0)
> 다음 글: [5편 — GitHub Actions OIDC 기반의 무자격증명(Credentialless) CI/CD 인프라 구축](https://velog.io/@yapp/%EB%B3%B4%EC%95%88%EC%9D%80-%EB%B0%B0%ED%8F%AC-%EC%A0%84%EC%97%90-%EC%8B%9C%EC%9E%91%EB%90%9C%EB%8B%A4-AWS-DevSecOps-CICD-%EC%84%A4%EA%B3%84%EA%B8%B0)
...
![](https://velog.velcdn.com/images/yapp/post/3f54f6e0-9abf-4801-abe7-26138cc4f030/image.png)
...
rule {
  ...
  statement {
    not_statement {
      statement {
        geo_match_statement {
          country_codes = ["KR"]![](https://velog.velcdn.com/images/yapp/post/18973f7e-c6a2-4cb7-950f-f6a274b47616/image.png)
...
tfsec을 로컬에서 실행해 배포 전 Terraform 코드를 검증했습니다. CI/CD 파이프라인에서는 aquasecurity가 tfsec을 deprecated하고 Trivy로 통합한 방향에 맞춰 Trivy IaC 스캔으로 동일한 커버리지를 제공합니다.
<!-- 사진 2: GitHub Actions 파이프라인 성공 화면 — Security Gates / Terraform / Deploy Lambda 3개 통과 -->
![](https://velog.velcdn.com/images/yapp/post/58e56fed-6339-49be-a026-9ddf4ab271f1/image.png)
...
![](https://velog.velcdn.com/images/yapp/post/2ed82110-e63e-4ed9-b4cb-7fcbcc038bac/image.png)
...
![](https://velog.velcdn.com/images/yapp/post/06da63a2-96fd-4bad-bc44-3c458b76ea7d/image.png)
...
다음 편에서는 이 인프라에 배포 자격증명 없이 접근하는 GitHub Actions OIDC 기반 CI/CD 파이프라인을 다룬다.
