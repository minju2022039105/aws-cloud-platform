# 탐지에서 차단까지 자동화하다 — SOAR 파이프라인과 AI 대시보드 구축기

> 이 글은 AWS DevSecOps 플랫폼 구축기 시리즈의 6편입니다.  
> 4편: [WAF가 놓친 것을 AI가 잡는 법](https://velog.io/@yapp/WAF가-놓친-것을-AI가-잡는-법)  
> 5편: [하루 $30이 사라졌다 — KMS 비용 폭증 사고 추적기](https://velog.io/@yapp/트러블슈팅-KMS-비용-폭증-사고)
...
**1) Athena `aiops_results`** — SOAR 파이프라인 실행 결과
```sql
-- AI가 탐지한 이상 IP 수 (WAF가 ALLOW한 트래픽 중)
...
- WAF 단독(400건) → WAF+AI(545건): 탐지 범위 **+36.3%**
- AI 추가 탐지 후보 146건: WAF가 ALLOW한 트래픽 중 AI가 이상 패턴으로 분류한 것
- Precision 57.97%: AI 탐지 결과 중 실제 공격 비율
- FPR 10.36%: 정상 트래픽을 잘못 차단하는 비율
...
```
monitor.py (시뮬레이션) → S3 → Athena → Lambda → WAF 차단
평가 결과 → S3 → Athena → Grafana (정적 KPI)
```
