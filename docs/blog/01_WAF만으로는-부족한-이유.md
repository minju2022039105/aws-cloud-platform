# AWS WAF만으로는 부족한 이유 — AI 이상 탐지를 붙이기까지
...
[인터넷 트래픽]
       ↓
  [AWS WAF v2]  ← AI가 찾아낸 위협 IP 즉각 차단 (Priority 1)
       ↓
  [ALB → EC2]                  S3 (WAF 로그 적재)
  (현재 구성)                        ↓
                             [Amazon Athena]  ← 로그 쿼리
                                   ↓
                   [Isolation Forest AI 엔진 — ai/training/]
                                   ↓
                   위협 탐지 → [Lambda] → WAF IP Set 업데이트
                                       → SNS 알림
```
...
> **실제로 수집된 로그를 보면**, 184건 중 180건이 GeoBlock에서 차단됐습니다.
> 해외 스캐너 트래픽이 이 정도로 많습니다. 이 180건이 Priority 2~4 룰까지 내려갔다면 그만큼 비용이 더 나왔을 겁니다.
...
1편(이 글): WAF의 한계와 AI 보완의 필요성, 전체 아키텍처 개요
