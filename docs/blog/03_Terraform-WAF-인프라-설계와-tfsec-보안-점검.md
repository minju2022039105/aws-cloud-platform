...
> 1편: [WAF만으로는 부족했다: AI 이상 탐지를 붙인 이유](https://velog.io/@yapp/WAF%EB%A7%8C%EC%9C%BC%EB%A1%9C%EB%8A%94-%EB%B6%80%EC%A1%B1%ED%96%88%EB%8B%A4-AI-%EC%9D%B4%EC%83%81-%ED%83%90%EC%A7%80%EB%A5%BC-%EB%B6%99%EC%9D%B8-%EC%9D%B4%EC%9C%A0)  
> 2편: [완벽한 WAF를 만들고 배포에서 무너지지 않는 법](https://velog.io/@yapp/%EB%B3%B4%EC%95%88%EC%9D%80-%EB%B0%B0%ED%8F%AC-%EC%A0%84%EC%97%90-%EC%8B%9C%EC%9E%91%EB%90%9C%EB%8B%A4-AWS-DevSecOps-CICD-%EC%84%A4%EA%B3%84%EA%B8%B0)
...
이전에 리소스마다 개별 KMS 키를 만들었다가 하루 $30 비용 폭탄을 맞은 경험에서 나온 설정입니다. 자세한 내용은 6편 트러블슈팅에서 다룹니다.

---

## tfsec — Terraform 보안 취약점 수동 점검

tfsec을 로컬에서 실행해 Terraform 코드를 수동으로 점검했습니다.

초기에는 CI Security Gates에 포함을 시도했으나 워크플로우 안정성 문제가 반복되어 최종 파이프라인에서는 제외했습니다. 최종 자동 Security Gates는 Trivy IaC 스캔과 Bandit Python 코드 분석으로 구성했고, tfsec은 수동 점검 및 취약점 개선 이력 관리 목적으로 활용했습니다.

<!-- 사진 2: GitHub Actions 파이프라인 성공 화면 — Security Gates(Trivy IaC + Bandit) / Terraform / Deploy Lambda 3개 통과 -->
...
![업로드중..](blob:https://velog.io/482eadc6-9847-44b1-a605-7719d11ca4cc)
